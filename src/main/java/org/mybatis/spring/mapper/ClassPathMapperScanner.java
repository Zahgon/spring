...
public class ClassPathMapperScanner extends ClassPathBeanDefinitionScanner {
    ...
    private void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
        ...
        for (BeanDefinitionHolder holder : beanDefinitions) {
            ...
            // issue #59
            var constructorArgument = new ConstructorArgumentValues.ValueHolder(beanClassName);
    ...
    private boolean shouldUseClassConstructorArgument(Class<? extends MapperFactoryBean> mapperFactoryBeanClass) {
        return hasSingleArgumentConstructorAccepting(mapperFactoryBeanClass, Class.class) && !hasSingleArgumentConstructorAccepting(mapperFactoryBeanClass, String.class);
    }

    private boolean hasSingleArgumentConstructorAccepting(Class<? extends MapperFactoryBean> mapperFactoryBeanClass, Class<?> argumentType) {
        return Arrays.stream(mapperFactoryBeanClass.getDeclaredConstructors()).anyMatch(constructor -> constructor.getParameterCount() == 1 && constructor.getParameterTypes()[0].isAssignableFrom(argumentType));
    }
